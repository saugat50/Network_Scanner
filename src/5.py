...
class NetworkScannerApp(App):
    ...
    def sniff_http_packet_popup(self, instance):
        input_layout = BoxLayout(orientation='horizontal', spacing=10, padding=10)
        ip_input = TextInput(hint_text="Enter IP address", multiline=False, size_hint=(0.7, 1))
        send_button = Button(text="Send", size_hint=(0.3, 1))
        ...
        def on_send(instance):
            ip_address = ip_input.text
            if ip_address:
                sniff_thread = threading.Thread(target=self.sniff_http_packets_and_show_popup, args=(ip_address,))
        ...
        input_layout.add_widget(send_button)
        popup = Popup(
            title="Sniff HTTP Packets",
            content=input_layout,
            size_hint=(0.8, 0.4)
        )
        popup.open()

    def sniff_http_packets_and_show_popup(self, ip_address):
        sniffed_data = self.sniff_http_packets(ip_address)
        
        # Use Clock.schedule_once to schedule the UI update in the main thread
        Clock.schedule_once(lambda dt: self.show_sniffed_data_popup(sniffed_data), 0)

    def show_sniffed_data_popup(self, sniffed_data):
        result_popup = Popup(
            title='HTTP Sniffing Results',
    ...
    def build(self):
        ...
        sniff_http_button = Button(text='Sniff HTTP Packets', size_hint=(1, 0.2))
        sniff_http_button.bind(on_release=self.sniff_http_packet_popup)
        layout.add_widget(sniff_http_button)
