...
class NetworkScannerApp(App):
    ...
    def scan_open_ports(self, ip_address):
        nm = nmap.PortScanner()
        try:
            nm.scan(ip_address, arguments='-sS', timeout=30)  # Increased timeout
            open_ports = []
    ...
    def enumerate_services(self, ip_range):
        nm = nmap.PortScanner()
        try:
            nm.scan(hosts=ip_range, arguments='-sV --host-timeout 60m')  # Increased timeout for service scan
            results = []
            for host in nm.all_hosts():
                if nm[host].state() == 'up':
    ...
    def enumerate_services_popup(self, instance):
        ...
        ip_input = TextInput(
            ...
            multiline=False, 
            size_hint=(0.7, 1),
            foreground_color=(0.5, 0.5, 0.5, 1)  # Grey example text color
        )

        def on_focus(instance, value):
            if value:
                ip_input.hint_text = ""
                ip_input.foreground_color = (0, 0, 0, 1)  # Black text
            else:
                ip_input.hint_text = "e.g., 192.168.1.0/24"
                ip_input.foreground_color = (0.5, 0.5, 0.5, 1)  # Grey text

        ip_input.bind(focus=on_focus)
        send_button = Button(text="Send", size_hint=(0.3, 1))
